...
class TelegramConnectView(APIView):
    """
    POST   /api/auth/me/telegram/connect/  — token orqali Telegram ulash
    DELETE /api/auth/me/telegram/connect/  — Telegram uzish
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        if not token:
            return Response({'code': 'MISSING_TOKEN', 'message': 'Token kiritilmadi'}, status=status.HTTP_400_BAD_REQUEST)
        tg_id = cache.get(f'tg_connect_{token}')
        if not tg_id:
            return Response({"code": "INVALID_TOKEN", "message": "Token topilmadi yoki muddati o'tgan"}, status=status.HTTP_400_BAD_REQUEST)
        if User.objects.filter(telegram_user_id=tg_id).exclude(id=request.user.id).exists():
            return Response({'code': 'ALREADY_LINKED', 'message': 'Bu Telegram akkaunt boshqa foydalanuvchiga ulangan'}, status=status.HTTP_409_CONFLICT)
        request.user.telegram_user_id = tg_id
        request.user.save(update_fields=['telegram_user_id'])
        cache.delete(f'tg_connect_{token}')
        return Response({'message': 'Telegram muvaffaqiyatli ulandi', 'user': UserSerializer(request.user).data}, status=status.HTTP_200_OK)
